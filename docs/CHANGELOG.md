...
- **Ctrl+C during the concurrent AI passes no longer runs every queued Claude call.** `extractor.py`'s `_generate_summaries_concurrently()` and `_generate_etas_concurrently()` submitted all futures up front, and an interrupt left the executor's default `shutdown(wait=True)` to run the entire remaining queue — each queued item spawning a fresh `claude` subprocess (up to `CLAUDE_SUMMARY_TIMEOUT`, default 120 s) after the user had already stopped the run. Both passes now cancel queued futures (`shutdown(wait=False, cancel_futures=True)`) on `BaseException` and re-raise, matching the fix that shipped for `kfj_task_extractor.apply_ai_etas()` in #167. ([#168](https://github.com/J-MaFf/clickup_task_extractor/issues/168))
...
- **Optional headless 1Password auth for the KFJ weekly sync.** With a 1Password service-account token in `OP_SERVICE_ACCOUNT_TOKEN`, `kfj_task_extractor.py` now resolves its `op://` secret references through the SDK fully headlessly — no desktop app, so the scheduled task can run "whether user is logged on or not" (machine locked or logged out). The token path is tried first when set (a locked machine's DesktopAuth attempts would only stall) and is deliberately called directly, because `auth.load_secret_with_fallback` short-circuits to the 1Password Environment path whenever `OP_ENVIRONMENT_ID` is set globally and never reaches the service-token vault lookup. Without the token nothing changes: DesktopAuth remains the default interactive path. Token storage (OS-level user env var via `setx`; never in `.env.kfj`, the repo, or task arguments) and the Task Scheduler switch are documented in the README's new "Unattended / headless scheduled runs" section. ([#166](https://github.com/J-MaFf/clickup_task_extractor/issues/166))
- **Work Location column in the KFJ weekly sheet sync.** `kfj_task_extractor.py` now writes a `Work Location` column between Branch and Priority (header: `Task | Company | Branch | Work Location | Priority | Status | ETA`), sourced from a ClickUp dropdown custom field named `Work Location` (options `Remote` / `On-Site`) on the KFI Jefferson list. The dropdown is resolved with the same `LocationMapper` mechanism as Branch, the option's display casing is preserved, and tasks where the field is absent or unset render a blank cell (no fallback or inference). The value is carried in `TaskRecord._metadata`, so the main extractor's Markdown/HTML/CSV exports are unaffected; the bold-header range in `write_to_sheet()` is now derived from `len(HEADER)`. The new column appears only on tabs generated after this change — old weekly tabs are never modified. ([#183](https://github.com/J-MaFf/clickup_task_extractor/issues/183))
...
- **KFJ Task Extractor** (`kfj_task_extractor.py`): standalone weekly sync that pulls all open tasks from the ClickUp "KFI Jefferson" list into the tracking Google Sheet.
  ...
  - Writes the header and task rows sorted by priority then ETA, normalized to the sheet's conventions (lowercase priority/status, date-only ETA). (The column list has since been extended — see the Work Location entry under [Unreleased] for the current columns.)
