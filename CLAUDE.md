...
- **Deferred + concurrent AI enrichment**: `_process_task` resolves only non-generative notes (ClickUp field / base notes) and a deterministic baseline ETA, stashing AI inputs in `TaskRecord._metadata` (`ai_fields`, and `eta_inputs` for due-date-less tasks). Two bounded-concurrency passes then run after task processing: `_generate_summaries_concurrently()` (Claude/Gemini summaries) and `_generate_etas_concurrently()` (AI ETA for tasks without a due date). Both use `AI_SUMMARY_CONCURRENCY` (default 4), cover interactive (selected) and non-interactive (all) modes, preserve order, and run outside the live `Progress` display (`_progress_context` is cleared first).
