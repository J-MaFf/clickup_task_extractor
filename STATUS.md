...
**2026-08-10 — KFJ sheet gains a Work Location column.** [#183](https://github.com/J-MaFf/clickup_task_extractor/issues/183): the weekly "KFI Jefferson current tasks" tab now includes a "Work Location" column (Remote / On-site) between Branch and Priority, sourced from a same-named ClickUp dropdown custom field on the KFI Jefferson list (blank when unset; field is created manually in the ClickUp UI — the v2 API cannot create field definitions). Scoped to `kfj_task_extractor.py` via `_metadata` so the main extractor's exports are unchanged. Spec: `specs/kfj-work-location-column.md`. 393 tests pass.
